...
- **Workflow skills live flat in `.agents/skills/`** as `<name>/SKILL.md` direct children; the `kit-*` family (inbox capture/promote, learning note, release) ships in the installer's `workflows` component. `.codebuddy/skills` is a symlink to `.agents/skills`, because CodeBuddy Code discovers project skills only there — add and edit skills under `.agents/skills/` and never as real files under `.codebuddy/`.
